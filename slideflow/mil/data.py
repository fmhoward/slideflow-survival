...
def build_dataset_surv(bags, targets, events, encoder, bag_size, clin_vars = None, risk = None, use_lens=False,  max_bag_size=None, dtype=torch.float32):
    assert len(bags) == len(targets)
    if clin_vars is not None:
        assert len(bags) == len(clin_vars)

    def _zip(bag, targets, events):
        features, lengths = bag
        if encoder is None:
            _targets = targets
            _events = events
            ...
            _events = events.squeeze()
        if use_lens:
            return (features, lengths, (_targets, _events))
        else:
            return (features, (_targets, _events))


    def _zip_clin(bag, targets, events, clin_vars):
        features, lengths = bag
        if encoder is None:
            _targets = targets
            _events = events
            _clin_vars = clin_vars
        else:
            _targets = targets.squeeze()
            _events = events.squeeze()
            _clin_vars = clin_vars.squeeze()
        if use_lens:
            return (_clin_vars, features, lengths, (_targets, _events))
        else:
            return (_clin_vars, features, (_targets, _events))

    if clin_vars is not None:
        print("Building clin model")
        dataset = MapDataset(
            _zip_clin,
            BagDataset(bags, bag_size=bag_size, max_bag_size=max_bag_size, dtype=dtype),
            EncodedDataset(encoder, targets),
            FloatDataset(events),
            FloatDataset(clin_vars)
        )
...
class FloatDataset(Dataset):
    def __init__(self, arr: npt.NDArray):
        self.arr = arr
