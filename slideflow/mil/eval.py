...
def predict_mil_survival(
    ...
    **kwargs
    ...
    if isinstance(model, str):
        model_path = model
        model, config = utils.load_model_weights(model_path, config, input_shape = input_shape, output_shape = output_shape)
        model.eval()
    ...
    # Prepare labels.
    labels, _ = utils.get_labels(dataset, outcomes, config.is_classification())
    events_dict, unique_events = utils.get_labels(dataset, event_header, config.is_classification())
    ...
    if aggregation_level == 'patient':
        ...
        slide_to_patient = dataset.patients()
        n_slide_bags = len(bags) 
        bags, y_true, _, _, _ = utils.aggregate_bags_by_patient_survival(bags, labels,  events_dict, clin_vars = clin_vars, risk = None, slide_to_patient = slide_to_patient)
        log.info(f"Aggregated {n_slide_bags} slide bags to {len(bags)} patient bags.")
...
def run_eval(
    ...
    **heatmap_kwargs
    ...
    if config.is_multimodal:
        ...
        df  = predict_mil(uq=uq, **predict_kwargs)
        y_att = None
