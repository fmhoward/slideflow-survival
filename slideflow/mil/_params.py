...
class RiskTertileBatchSampler(BatchSampler):

    def __init__(self, risk, batch_size, shuffle=True):
        self.batch_size = batch_size
        self.shuffle = shuffle

        risks = np.asarray(risk)
        q1, q2 = np.quantile(risks, [1/3, 2/3])

        self.groups = [
            np.where(risks <= q1)[0],
            np.where((risks > q1) & (risks <= q2))[0],
            np.where(risks > q2)[0]
        ]
...
class TrainerConfig:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tertile = tertile
        if isinstance(model, str):
            self.model_config = build_model_config(model, tertile = tertile, **kwargs)
        else:
    ...
    def _build_dataloader_surv(
        ...
        return self.model_config._build_dataloader_surv(
            bags,
            targets,
            events,
            clin_vars,
    ...
    def build_train_dataloader_surv(
        self,
        bags,
        targets,
        events,
        ...
        *,
        ...
        if (dataloader_kwargs['batch_size'] > 1) and (self.tertile == True):
          assert not (risk is None), "Risk must be provided to train with tertiles"        

        return self._build_dataloader_surv(
            bags,
            targets,
            events,
            clin_vars,
...
class MILModelConfig:
    ...
    def inspect_batch(self, batch, clin_path = False) -> Tuple[int, int]:
        """Inspect a batch of data.
    ...
    def _build_dataloader_surv(
        ...
        *,
        ...
        if (dataloader_kwargs['batch_size'] > 1) and (self.tertile == True):
          ...
          dataset, risk = data_utils.build_dataset_surv(bags, targets, events, clin_vars = clin_vars, risk = risk, encoder=encoder, **dataset_kwargs)
          print("Using Tertile Sampler")
          batch_size = dataloader_kwargs.pop("batch_size")
          dataloader_kwargs.pop("device", None)
          print(dataloader_kwargs)
          dataloader_kwargs.pop("shuffle", None)
          dataloader_kwargs.pop("drop_last", None)
          
          sampler = RiskTertileBatchSampler(
              risk=risk,
          ...
          dataset = data_utils.build_dataset_surv(bags, targets, events, clin_vars = clin_vars, risk = risk, encoder=encoder, **dataset_kwargs)          
          dataloader = DataLoader(dataset, **dataloader_kwargs)
        return dataloader
