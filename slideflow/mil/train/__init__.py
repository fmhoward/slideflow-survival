...
def build_fastai_learner_survival(
    config: TrainerConfig,
    train_dataset: Dataset,
    val_dataset: Dataset,
    outcomes: Union[str, List[str]],
    ...
    bags: Union[str, np.ndarray, List[str]],
    *,
    clin_vars: Union[str, List[str]] = None,
    risk = None,
    split_times = None,
    outdir: str = 'mil',
    return_shape: bool = False,
    **kwargs
    ...
    labels, unique = utils.get_labels((train_dataset, val_dataset), outcomes, config.is_classification())    
    if clin_vars:
        clin_vars, _ = utils.get_labels((train_dataset, val_dataset), clin_vars, False)
    if risk:
        risk, _ = utils.get_labels((train_dataset, val_dataset), risk, False)
    
    events_dict, unique_events = utils.get_labels((train_dataset, val_dataset), event_header, config.is_classification())
    ...
    if config.aggregation_level == 'slide':
        # Aggregate feature bags across slides.
        bags, targets, events, clin_vars, risk, train_idx, val_idx = utils.aggregate_trainval_bags_by_slide_survival(
            bags,  # type: ignore
            labels,
            events_dict,
            train_slides,
        ...
        # Aggregate feature bags across patients.
        n_slide_bags = len(bags)
        bags, targets, events, clin_vars, risk, train_idx, val_idx = utils.aggregate_trainval_bags_by_patient_survival(
            bags,  # type: ignore
            labels,
            events_dict, 
            train_slides,
    ...
    print(risk)
    # Build FastAI Learner
    learner, (n_in, n_out) = _fastai.build_learner_survival(
        config,
        bags=bags,
        targets=targets,
        events=events,
