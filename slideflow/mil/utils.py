...
def aggregate_bags_by_patient_survival(
    bags: np.ndarray,
    labels: Dict[str, int],
    events: Dict[str, int],
    clin_vars: Union[Dict[str, int], Dict[str, list]],
    risk: Dict[str, float],
    slide_to_patient: Dict[str, str],
) -> Tuple[np.ndarray, np.ndarray]:
    """Aggregate bags by patient.
    ...
    for patient, patient_bags in patient_to_bags.items():
        ...
        first_bag = patient_bags[0] if isinstance(patient_bags[0], str) else patient_bags[0][0]
        patients_labels[patient] = labels[path_to_name(first_bag)]
        patients_events[patient] = events[path_to_name(patient_bags[0])]
        if clin_vars:
            patient_clinvars[patient] = clin_vars[path_to_name(patient_bags[0])]
        if risk:
            patient_risk[patient] = risk[path_to_name(patient_bags[0])]            
    # Prepare targets, mapping each bag sublist to the label of the first bag.
    targets = np.array([patients_labels[slide_to_patient[path_to_name((sublist[0] if isinstance(sublist[0], str) else sublist[0][0]))]]
                        for sublist in bags])
    _events = np.array([patients_events[slide_to_patient[path_to_name(sublist[0])]]
                        for sublist in bags])
    _clin_vars = None
    if clin_vars:
        _clin_vars = np.array([patient_clinvars[slide_to_patient[path_to_name(sublist[0])]]
                        for sublist in bags])
    _risk = None
    if risk:
        _risk = np.array([patient_risk[slide_to_patient[path_to_name(sublist[0])]]
                        for sublist in bags])        
    return bags, targets, _events, _clin_vars, _risk
...
def aggregate_trainval_bags_by_slide_survival(
    bags: np.ndarray,
    ...
    *,
    ...
    # Prepare targets
    targets = np.array([labels[path_to_name(f)] for f in bags])
    _events = np.array([events[path_to_name(f)] for f in bags])
    if clin_vars:
        _clin_vars = np.array([clin_vars[path_to_name(f)] for f in bags])
    else:
        _clin_vars = None
    
    if risk:
        _risk = np.array([risk[path_to_name(f)] for f in bags])
    else:
        _risk = None
    ...
    return bags, targets, _events, _clin_vars, _risk, train_idx, val_idx


def aggregate_trainval_bags_by_patient_survival(
    bags: np.ndarray,
    labels: Dict[str, float],
    events: Dict[str, int],
    train_slides: List[str],
